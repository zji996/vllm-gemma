...
from vllm.model_executor.layers.sparse_mask_attention import (
    sparse_mask_attention_forward,
)
...
class MMEncoderAttention(CustomOp):
    ...
    def _forward_sparse_mask(
        ...
        """Route ViT attention through the sparse-mask backend when enabled."""
        bsz, q_len = query.size()[:2]
        kv_len = key.size(1)
        is_reshaped = query.dim() != 4
        ...
        output = sparse_mask_attention_forward(
            query=query,
            key=key,
            value=value,
            scale=self.scale,
    ...
    def forward_native(
        ...
        if self.attn_backend == AttentionBackendEnum.CUSTOM:
            return self._forward_sparse_mask(query, key, value, cu_seqlens)
